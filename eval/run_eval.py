...
from .corpus import CORPUS, REFUSAL_PHRASES, EvalDocument, EvalQuestion
...
def ask(base_url: str, question: str) -> tuple[str, list[str], float]:
    ...
    @returns Tuple of (full answer, source filenames in retrieval order, latency seconds).
    """
...
def score(
    ...
    For in-corpus questions, `passed` requires both substring match AND
    top-1 retrieval. For out-of-corpus probes, `passed` requires a refusal.

    @param document Source document context (or the OOC placeholder).
    @param question Canned question.
    @param answer   Model output.
    @param sources  Filenames cited in the streamed response, in order.
    ...
    passed = refused if question.ooc else (answer_match and top_source_correct)
...
def run(base_url: str, skip_download: bool) -> list[AnswerScore]:
    ...
    for document in CORPUS:
        for question in document.questions:
            label = "ooc" if question.ooc else document.filename
            print(f"[ask] {label}: {question.prompt}")
